# Mirror — Personal AI Journal Bot (Build Plan)

> Drop this file into your project folder as `CLAUDE.md` before starting a new Claude session.
...
**Current session:** Session 0 (not started yet)
**Build order:** Session 0 (setup) → Session 1 (core bot) → Session 2 (vision + questions) → Session 3 (onboarding) → Session 4 (reports + commands) → Session 5 (deploy)
...
```
mirror_bot.py          # Main bot (~500-600 lines)
mirror_memory.py       # 3-tier memory management (~250 lines)
mirror_prompts.py      # All Claude prompts (~150 lines)
mirror_vision.py       # Journal photo OCR + edit flow (~120 lines)
mirror_reports.py      # HTML report generation (~200 lines)
...
- Install 7 skills from aitmpl.com:
  1. `skills/enterprise-communication/telegram-bot-builder`
  2. `skills/ai-research/agent-memory-systems`
  3. `skills/ai-research/conversation-memory`
  4. `skills/database/supabase-postgres-best-practices`
  5. `agents/database/supabase-schema-architect`
  6. `skills/development/claude-api`
  7. `skills/ai-research/context-window-management`
- Install 2 MCP servers:
  1. Supabase MCP: `claude mcp add --transport http supabase https://mcp.supabase.com/mcp?project_ref=YOUR_REF`
  2. Context7: `claude mcp add --scope user --transport stdio context7 -- cmd /c npx -y @upstash/context7-mcp@latest`
